...
最新行情
```
→ 自动拉取隔夜美股收盘数据 + 今日 A 股开盘/盘中要闻 + 全球大宗商品

---

## 📰 最新行情 (Global Market Brief)

当用户输入 "最新行情" 时，自动执行以下流程：

### 数据采集

| 类别 | 内容 | 数据源 |
|------|------|--------|
| **隔夜美股** | 道指/纳指/标普/费城半导体收盘涨跌、龙头个股表现、重大事件 | websearch_aisearch |
| **全球商品** | WTI原油、黄金、白银、美债收益率 | websearch_aisearch |
| **今日 A 股** | 三大指数开盘/盘中、板块涨跌、涨跌比、热点题材 | websearch_aisearch |
| **要闻速递** | 政策发布、行业动态、地缘事件 | websearch_aisearch |
| **ETF 实时行情** | 持仓和关注列表的实时价格 | qt.gtimg.cn |

### 执行步骤

```
1. websearch_aisearch(query="美股收盘 {今日日期} 道指 纳指 半导体 原油 黄金")
2. websearch_aisearch(query="A股开盘 {今日日期} 早盘热点 板块 政策")
3. curl qt.gtimg.cn/q={所有持仓+关注代码}
4. 解析数据，输出统一格式
```
...
**全球商品**:
- WTI 原油 $XX (+X%) / 黄金 $X,XXX / 10Y 美债 X.XX%
